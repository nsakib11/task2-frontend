...
import { designationSchema } from "../components/ValidationSchemas";
...
const AddDesignation = () => {
  const {
    ...
    resolver: yupResolver(designationSchema), // Use the designation schema for validation
  });
        ...
        if (responseData.includes("already exists")) {
          if (responseData.includes("code")) {
            setApiFieldErrors((prev) => ({
              ...prev,
              code: "Designation with this code already exists",
            }));
          }
          if (responseData.includes("name")) {
            setApiFieldErrors((prev) => ({
              ...prev,
              name: "Designation with this name already exists",
            }));
          }
        } else {
